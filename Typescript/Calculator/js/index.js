...
class Calculator {
    ...
    // trigonometry
    computeSin() {
        this.output.innerText = Math.sin(Number(eval(this.input.value)) || 0).toString();
        this.input.value = "sin(" + `${this.input.value || 0}` + ")";
    }
    computeCos() {
        this.output.innerText = Math.cos(Number(eval(this.input.value)) || 0).toString();
        this.input.value = "cos(" + `${this.input.value || 0}` + ")";
    }
    computeTan() {
        this.output.innerText = Math.tan(Number(eval(this.input.value)) || 0).toString();
        this.input.value = "tan(" + `${this.input.value || 0}` + ")";
    }
    computeCosec() {
        this.output.innerText = (1 / Math.sin(Number(eval(this.input.value)) || 0)).toString();
        this.input.value = "cosec(" + `${this.input.value || 0}` + ")";
    }
    computeSec() {
        this.output.innerText = (1 / Math.cos(Number(eval(this.input.value)) || 0)).toString();
        this.input.value = "sec(" + `${this.input.value || 0}` + ")";
    }
    computeCot() {
        this.output.innerText = (1 / Math.tan(Number(eval(this.input.value)) || 0)).toString();
    ...
    // Sqrt
    computeSqrt2() {
        this.output.innerText = Math.sqrt(parseInt(this.input.value) || 0).toString();
    ...
    // Ceil
    computeCeil() {
        this.output.innerText = Math.ceil(Number(eval(this.input.value)) || 0).toString();
        this.input.value = "ceil(" + `${this.input.value || 0}` + ")";
    }
    // Floor
    computeFloor() {
        this.output.innerText = Math.floor(Number(eval(this.input.value)) || 0).toString();
    ...
    // ABS
    computeAbs() {
        this.output.innerText = Math.abs(Number(eval(this.input.value)) || 0).toString();
...
BtnGroup.deg.addEventListener("click", () => {
    BtnGroup.deg.innerHTML =
        BtnGroup.deg.innerHTML === "RAD" ? "DEG" : "RAD";
});
